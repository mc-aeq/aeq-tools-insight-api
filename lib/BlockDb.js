...
BlockDb.prototype._addStxsScript = function(stxs, hash, height) {
  var dbScript = [];

  for (var ii in stxs) {
    dbScript.push({
      type: 'put',
      key: IN_BLK_PREFIX + stxs[ii],
      value: hash + ':' + height,
    });
  }
  return dbScript;
...
BlockDb.prototype.add = function(b, height, cb) {
  ...
  this.fromHashWithInfo(b.hash, function(err, bi) {
	if (!bi || !bi.info || !bi.info.tx)
	  info('unable to get info for block:' + hash);	
	SlackPush.push(bi);
  });
};
