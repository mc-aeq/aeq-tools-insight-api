...
var SlackPush = require('./SlackPush');
...
HistoricSync.prototype.updatePercentage = function() {
  ...
  if( this.syncPercentage == 100 ) SlackPush.push('Sync Done, only pushing new blocks now.');
};
    ...
    self.sync.bDb.getTip(function(err, tip, height) {
      ...
      async.doWhilst(
        ...
        function(err) {
          ...
          var limit = self.network == 'aeqdlivenet' ? 6 : 100;
          if (d > limit) {
...
HistoricSync.prototype.showSyncStartMessage = function() {
  ...
  SlackPush.push('Doing Sync, this may take a while and produce a lot of messages')
  self.showProgress();
};
